  function [Clu]=Cluster_Ad_Hoc_Network_ACO_V3_1;
 %%%it is second version ACO algorithm. ------ACO#2
...
for kk=first_ant:ANT_Number
 ...
 while Uncovered_Node
         ...
         Feasiable=[];
         Feasiable=FindFeasible_V1(ANT(kk)); %%%%Here, we choose the feasiable nodes to choose as cluster heads.  
         if   numel(Feasiable)==0
            Uncovered_Node==0;
            break;
         end   
         if it==1
         [next_ind,next_node]=TransProbaility_First_Iteration_V3_1(ANT(kk),Feasiable,ANT(kk).Sequence);
         else

         [next_ind,next_node]=TransProbaility_NextIteration_V3_1(ANT(kk),Feasiable,tau);
         end
         ...
         ANT(kk).CoverNodes(end:end+index+1)=[Neighbor{ANT(kk).Sequence(end)},SelectedNode];
         ANT(kk).NoCoverNodes=numel(unique(ANT(kk).CoverNodes))+1;
 ...
 COSTFUNCTION_for_CLUSTERING= COST_ANT_V3_1(ANT(kk).Sequence);
 %=========================COST
...
Delta_Tau_NODE=UpdatedPheromon_V3_1(BEST_ANT,BEST_ANT_for_Iteartion(it),tau);
...
BEST_Solution=BEST_ANT.Sequence;
  ...
  [Clu]=Form_Cluster_Ad_Hoc_Network_for_ACO(BEST_Solution);
% plot( OTHER_COST(:,1),'--b')
% figure;
