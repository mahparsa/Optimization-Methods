   ...
   function Population_for_MinChannel = GeneratPopulation_MinChannel_Neighbor_Adjacency1 ;
...
p=AlgorithmParams.PopSize ;
UPPER=min(N,MaxChannel)
HHH=zeros(p,N+UPPER); %%%%the chromosom consists of two parts, 
...
for i=1:p
    ...
    Number_CH=min(randint(1,1,[MinChannel+1 MaxChannel]),N); %%%% for each individulal a number of channel is considered.Number channel shows how many color is yuilized for this chromozom.   
    ...
    First_CH_Cluster=HHH(i,N+1:N+Number_CH);%%% choosing channel to assing the cluster. one channel to one cluster
     ...
     %%%%Which channel is assined to wich cluster
      ...
      for ii=1:size(Value,2)
           ...
           Index1=Index_Adjacent_Cluster{1,1};
              NVauel_CH=[];
              NAssined_CH=[];
  ...
  if  size(Set_Channel_To_Adjacent_Cluster,2)>=1
            ...
            m=randint(1,1,[1,size(Remained_CH,2)]);
