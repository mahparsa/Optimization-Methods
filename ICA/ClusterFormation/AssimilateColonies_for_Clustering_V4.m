...
function TheEmpire = AssimilateColonies_for_Clustering_V4(TheEmpire)
...
for j=1:Number_of_Assimialted_Colony
     ...
     DIGREE_IMPERIALIST=INT_of_NODE_in_Sequence( DifferentNoResource_IMPERILST);
     DIGREE_COLONY=INT_of_NODE_in_Sequence( DifferentNoResource_COLONY);
   
     
     node=[];Index_node=[];
     [min_nei,Index_node]=max( DIGREE_COLONY);
     [VAL,IND]=find(TEMP==DifferentNoResource_COLONY(Index_node));
     
     node=[];Index_node=[];
     [max_nei,Index_node]=min(  DIGREE_IMPERIALIST );
