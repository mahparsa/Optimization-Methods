 ...
 function [CHParams]=Form_Cluster_Ad_Hoc_Network_for_ICA(BEST_Solution);
...
Node.Neighbors=ProblemParams.NODE.Neighbors;
Node.ClusterAhead=zeros(1,Node.Number);
...
%-----------------------------------------------------:::::
% Node.Cluster_Neighbor shows the cluster ID of neighbors
...
for j=1:Node.Cluster_Number
      ...
      if numel( unique(Node.Cluser_Gatway{j}))<2
...
v=Node.Cluster_NO_Neighbor(i_M);
  ...
  [j,jj]=setdiff(Node.Cluster_Neighbor{i_M},randperm(Node.Cluster_Number));
  [j1,jj1]=setdiff(i_M,jj);
 [vv,ind]=sort(distance(Node.Cluster_ID(i_M),Node.Cluster_ID(j1)),'ascend');
...
i1=0;
v1=0;
 [v1,i1]=intersect(Node.Cluser_Gatway{ii}, Node.Cluster_ID);
 if numel(i1)~=0


    Node.Cluser_Gatway{ii}(i1)=[];
   
   
    [v3,i3]=intersect( Node.Cluster_Neighbor{ii},i1);
    Node.Cluster_Neighbor{ii}(i3)=[];
...
% 
% 
CHParams.Number_of_Cluster=Node.Cluster_Number;                       
CHParams.Member_of_Cluster=Node.Cluster_Member; 
CHParams.Index_of_Cluster=Node.Cluster_ID;
CHParams.Cluster_Ahead_of_Node=Node.ClusterAhead;
CHParams.Distance_Cluster_Ahead=Node.Dist_Cluster_Ahead;                       
CHParams.Cluster_Interfrence=Node.Cluster_INT; 
CHParams.Cluster_Gatway=Node.Cluser_Gatway;
CHParams.Cluster_Neighbor= Node.Cluster_Neighbor; 
