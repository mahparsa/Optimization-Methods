...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
AlgorithmParams.NumOfCountries = round(ProblemParams.NODE.Number/5);               % Number of initial countries.number of population
...
%%========================COST=================================== 
COST_Country_for_Clustering_V4;
...
for ii = 1:numel(Empires)
         ...
         Empires(ii)=AssimilateColonies_for_Clustering_V4(Empires(ii));
          %%========================COST=================================== 
         Empires(ii)=COST_Empires_for_Clustering_V4(Empires(ii));
          %%========================COST=================================== 
         Val=[];
         SortInd=[];
...
if AlgorithmParams.RevolutionRate2>0.2       
         ...
         %%========================COST=================================== 
         Empires(Index)=COST_Empires_for_Clustering_V4(Empires(Index));
          Empires(Index)=COST_Imperialist_for_Clustering_V4(Empires(Index));
         %%========================COST=================================== 
         if numel(Empires(Index).Colonies)
          
                     Empires(Index).TotalCost = Empires(Index).Imperialist.Cost + AlgorithmParams.Zeta * mean([Empires(Index).Colonies(:).Cost]);

         else 
              Empires(Index).TotalCost = Empires(Index).Imperialist.Cost;
         end
...
elseif AlgorithmParams.RevolutionRate1>0
         ...
         for kk=1:No_Imperialist_For_revolotion
              ...
              Empires(New_Index)=RevolveColonies_for_Clustering_Level1_V4(Empires(New_Index));
         %%========================COST=================================== 
         Empires(New_Index)=COST_Empires_for_Clustering_V4(Empires(New_Index));
          Empires(New_Index)=COST_Imperialist_for_Clustering_V4(Empires(New_Index));
         %%========================COST=================================== 
         if numel(Empires(New_Index).Colonies)
          
             Empires(New_Index).TotalCost = Empires(New_Index).Imperialist.Cost + AlgorithmParams.Zeta * mean([Empires(New_Index).Colonies(:).Cost]);
 
          else 
              Empires(New_Index).TotalCost = Empires(New_Index).Imperialist.Cost;
          end
 ...
 for jj=1: numel(Empires)
          Empires(jj)=COST_Empires_for_Clustering_V4(Empires(jj));
          Empires(jj)=COST_Imperialist_for_Clustering_V4(Empires(jj));
          if numel(Empires(jj).Colonies)
          
              Empires(jj).TotalCost = Empires(jj).Imperialist.Cost + AlgorithmParams.Zeta * mean([Empires(jj).Colonies(:).Cost]);
 
          else 
              Empires(jj).TotalCost = Empires(jj).Imperialist.Cost;
          end
 end    
...
%      Min_Other_FUN(Decade,:)=Other_COST(:,I);%%%it means that it is the value of other cost function  when the cost function is minimum. 
   ...
   MeanCost(Decade) = mean([Empires(:).TotalCost]);
      COST_int(Decade)=COST_INTERFRENCE(Empires(I).Imperialist.Sequence);
   COST_con(Decade)=COST_Connectivity_V2(Empires(I).Imperialist.Sequence);
...
INIMUM_COST(S,:)=BestCosts_for_MinChannel;
MEAN_COST(S,:)=MinTotalCost;
INTEFRENCE(S,:)= COST_int;
CONNECTIVITY(S,:)= COST_con;
